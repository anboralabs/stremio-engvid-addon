...
public class AddonRepositoryImplTest {
  ...
  public void setUp() throws Exception {
    ...
    ListLessonInfoToVOMapper listLessonInfoToVOMapper =
        new ListLessonInfoToVOMapper(lessonInfoToVOMapper);
    LessonVOMapper lessonVOMapper = new LessonVOMapper();
    ListLessonVOMapper listLessonVOMapper =
        new ListLessonVOMapper(lessonVOMapper);

    this.addonRepository =
        new AddonRepositoryImpl(mediaToVOMapper, listLessonInfoToVOMapper,
  ...
  public void givenLessonMediaSaveInLocalDB() {
    ...
    LessonMediaVO lessonMediaVO =
        new LessonMediaVO(1L, "image.jpg", "youtubeId", true);
  ...
  public void givenListLessonInfoSaveInLocalDB() {
    ...
    LessonInfo lessonInfo =
        LessonInfo.builder().id(1).title("test").description("test").build();

    LessonInfoVO lessonInfoVO =
        new LessonInfoVO(1, "test", "test", null, null, null, null);
  ...
  public void getAllLessonFromLocalDB() {
    ...
    LessonVO lessonVO = new LessonVO(1L, "test", "test", null, null, null, null,
                                     null, "youtubeId", null);
    ...
    Mockito.when(lessonDao.findAll()).thenReturn(Arrays.asList(lessonVO));

    Assert.assertEquals(Arrays.asList(lesson),
                        this.addonRepository.getLessons());
  }
  ...
  public void getLessonsByCategoryFromLocalDB() {
    ...
    Assert.assertEquals(Arrays.asList(lesson),
                        this.addonRepository.getLessonsByCategory(
                            EnglishLevel.BEGINNER.getId()));
  }
  ...
  public void givenAValidIdLessonReturnLesson() {
    ...
    Mockito.when(lessonDao.findById(idLesson)).thenReturn(lessonVO);
  ...
  public void givenAnInvalidIdLessonReturnNull() {
    ...
    Mockito.when(lessonDao.findById(Mockito.anyInt())).thenReturn(null);
