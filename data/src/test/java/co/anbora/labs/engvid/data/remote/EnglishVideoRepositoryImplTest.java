...
public class EnglishVideoRepositoryImplTest {
  ...
  public void setUp() throws Exception {
    ...
    listLessonInfoDTOMapper =
        new ListLessonInfoDTOMapper(new LessonInfoDTOMapper());
    repository = new EnglishVideoRepositoryImpl(api, listLessonInfoDTOMapper,
                                                htmlToLessonMediaMapper);
  }
  ...
  @Test
  public void givenARequestAllLessonReturnLessonsAvailable()
      throws IOException {
    ...
    Assert.assertEquals(Arrays.asList(lessonInfo, lessonInfo2),
                        repository.getLessons());
  }

  @Test(expected = LessonNotFoundException.class)
  public void givenAnInvalidSlugThrowsLessonNotFoundException()
      throws IOException {
    ...
    Response errorCode = new Response.Builder()
                             .request(request)
    ...
    Mockito.when(api.getMediaInfoBySlug("mock-lesson")).thenReturn(errorCode);

    repository.getLessonMediaById("mock-lesson", invalidId);
  }

  @Test
  public void givenAValidSlugReturnLessonMedia()
    ...
    String html = Files
                      .lines(Paths.get(
                          classLoader.getResource("lesson_media.html").toURI()))
    ...
    Mockito.when(api.getMediaInfoBySlug(mediaSlug)).thenReturn(okResponse);
    ...
    Assert.assertEquals(media,
                        repository.getLessonMediaById(mediaSlug, lessonId));
  }
}
