...
public class GetAllLessonsUseCaseTest {
  ...
  public void givenAnInvalidCatalogIdReturnEmptyResponse() {
    ...
    GetAllLessonsUseCase.Request request = new GetAllLessonsUseCase.Request(
        StremioConstants.StremioCatalog.MOVIE, invalid, search);
    GetAllLessonsUseCase.Response response =
        getAllLessonsUseCase.execute(request);
  ...
  public void givenBeginnerIdCatalogIdReturnAllBeginnerLessons() {
    ...
    Mockito
        .when(repository.getLessonsByDescription(EnglishLevel.BEGINNER.getId(),
                                                 searchValue))
        .thenReturn(CommonValuesForTests.beginnerLessons());

    GetAllLessonsUseCase.Request request = new GetAllLessonsUseCase.Request(
        StremioConstants.StremioCatalog.MOVIE,
        StremioConstants.StremioCatalog.BEGINNER_ID_CATALOG, search);
    GetAllLessonsUseCase.Response response =
        getAllLessonsUseCase.execute(request);
    Assert.assertEquals(response.getLessons(),
                        CommonValuesForTests.beginnerLessons());
  }
  ...
  public void givenIntermediateIdCatalogIdReturnAllIntermediateLessons() {
    ...
    Mockito
        .when(repository.getLessonsByDescription(
            EnglishLevel.INTERMEDIATE.getId(), searchValue))
    ...
    GetAllLessonsUseCase.Response response =
        getAllLessonsUseCase.execute(request);
    Assert.assertEquals(response.getLessons(),
                        CommonValuesForTests.intermediateLessons());
  }
  ...
  public void givenAdvancedIdCatalogIdReturnAllAdvancedLessons() {
    ...
    Mockito
        .when(repository.getLessonsByDescription(EnglishLevel.ADVANCE.getId(),
                                                 searchValue))
    ...
    GetAllLessonsUseCase.Response response =
        getAllLessonsUseCase.execute(request);
    Assert.assertEquals(response.getLessons(),
                        CommonValuesForTests.advancedLessons());
  }
}