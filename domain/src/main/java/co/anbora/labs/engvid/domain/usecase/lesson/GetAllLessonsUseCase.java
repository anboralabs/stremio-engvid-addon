...
public class GetAllLessonsUseCase
    extends UseCase<GetAllLessonsUseCase.Request,
                    GetAllLessonsUseCase.Response> {
  ...
  private Response getResponse(String searchValue, EnglishLevel englishLevel) {
    if (searchValue.isEmpty()) {
      return new Response(
          repository.getLessonsByCategory(englishLevel.getId()));
    }
    return new Response(
        repository.getLessonsByDescription(englishLevel.getId(), searchValue));
  }
